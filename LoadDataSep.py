...
class TauIdDataset(InMemoryDataset):
    ...
    def __init__(self, root, mode='train', num = 1024,transform=None, pre_transform=None):
        ...
        # self.cat_type1 = CategoricalDtype(categories=[0, 1, 10], ordered = True)
        # self.cat_type2 = CategoricalDtype(categories=[-1, 0, 1], ordered=True)
        # self.cat_type3 = CategoricalDtype(categories=[-1, 0, 1, 2], ordered=True)
        # self.cat_type4 = CategoricalDtype(categories=[1, 2, 11, 13, 130, 211, 22], ordered=True)
        # self.cat_type5 = CategoricalDtype(categories=[1, 5, 6, 7], ordered=True)
        # self.cat_type6= CategoricalDtype(categories=[1, 2, 3], ordered=True)
    ...
    def process(self, root_file, file_name):
        ...
        #      _df[feature] = df[feature]
        # for feature in VECT_FEATURES:
        #     _df[feature] = _df[feature].apply(lambda x: x[0])
        #     if _df[feature].dtype == 'object':
        #         _df[feature] = _df[feature].astype('float32')
        # df_vars = Data()
        # df_vars.x = torch.from_numpy(_df.values)
        ...
        # for feature_index in range(len(CATEGORICAL_FEATURES)):
