...
from keras.layers import Conv1D, Dense, Flatten, Input, MaxPooling1D, Layer
...
def build_model(input_shape, num_classes):
    x = Input(input_shape)
    y = Conv1D(1024,kernel_size=4, activation='relu')(x)
    y = MaxPooling1D()(y)
    y = Conv1D(512,kernel_size=4, activation='relu')(y)
    y = MaxPooling1D()(y)
    y = ODEBlock(64, 4)(y)
    y = Flatten()(y)
